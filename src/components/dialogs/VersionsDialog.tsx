...
const VersionsDialog = forwardRef<HTMLDialogElement, Props>(({ locale }, ref) => {
  ...
  const chipLabel = (type: 'stable' | 'beta' | 'alpha') => {
    const map = { stable: locale === 'es' ? 'Estable' : 'Stable', beta: 'Beta', alpha: 'Alpha' };
    return map[type];
  };
    ...
    <>
      ...
      <dialog ref={ref} className="glass-dialog glass-dialog--large" onClick={handleBackdrop}>
        <div className="dialog-header">
          <h3 className="text-[#e5e1e7] font-medium text-[22px] leading-7">{t('versions.title')}</h3>
          ...
          {loading ? (
            ...
            <div>
              {releases.map((release, i) => {
                ...
                const url = release.assets[0]?.browser_download_url ?? buildDownloadUrl(REPOS.android, release.tag_name);
                return (
                  <div key={release.tag_name} className="version-list-item">
                    <div className="version-list-info">
                      <div className="version-meta-row">
                        ...
                        {i === 0 && <span className="version-chip version-chip--latest">{locale === 'es' ? 'Más reciente' : 'Latest'}</span>}
                        <span className={`version-chip version-chip--${type}`}>{chipLabel(type)}</span>
                      </div>
                      <span className="version-date">{dateStr}</span>
                    </div>
                    <a href={url} className="version-dl-btn" target="_blank" rel="noopener noreferrer">
